...
func Erc721MintMany(t *testing.T, client *ethclient.Client, opts *bind.TransactOpts, erc721Contract common.Address, tokens []Erc721) {
	for _, tok := range tokens {
		Erc721Mint(t, client, opts, erc721Contract, tok.Id, tok.Metadata[:])
	}
}
...
func Erc721ApproveMany(t *testing.T, client *ethclient.Client, opts *bind.TransactOpts, erc721Contract, recipient common.Address, tokens []Erc721) {
	for _, tok := range tokens {
		Erc721Approve(t, client, opts, erc721Contract, recipient, tok.Id)
	}
}

func Erc721AssertOwner(t *testing.T, client *ethclient.Client, opts *bind.TransactOpts, erc721Contract common.Address, tokenId *big.Int, expected common.Address) {
	addr, err := utils.OwnerOf(client, opts, erc721Contract, tokenId)
	if err != nil {
		t.Fatal(err)
	}
...
func Erc721AssertMetadata(t *testing.T, client *ethclient.Client, opts *bind.TransactOpts, erc721Contract common.Address, tokenId *big.Int, expected string) {
	actual, err := utils.Erc721GetTokenURI(client, opts, erc721Contract, tokenId)
	if err != nil {
		t.Fatal(err)
	}
