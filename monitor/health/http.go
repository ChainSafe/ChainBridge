...
func (s httpMetricServer) healthStatus(w http.ResponseWriter, r *http.Request) {
	...
	for _, chain := range chains {
		...
		if prevHeight, ok := s.blockheights[chain.Id()]; ok {
			...
			timeDiff := latestBlock.Timestamp.Sub(prevHeight.lastUpdated)
			if latestBlock.Height.Cmp(prevHeight.height) >= 0 && int(timeDiff.Seconds()) < s.timeDelay {
				...
				if int(timeDiff.Seconds()) > s.timeDelay {
					...
					response := &httpResponse{
						Data:  "",
						Error: fmt.Sprintf("%s%s%s%s", "Chain height hasn't changed in: ", timeDiff, " Current height: ", latestBlock.Height),
					}
					w.WriteHeader(http.StatusInternalServerError)
					err := json.NewEncoder(w).Encode(response)
					if err != nil {
						log.Error("Failed to write, chain height hasn't changed in: %d seconds, Current height: %d", timeDiff.Seconds(), latestBlock.Height, err)
					}
					return
				} else {
					...
					response := &httpResponse{
						Data:  "",
						Error: fmt.Sprintf("%s%s%s%s%s", "latestBlock is <= previousHeight ", "previousHeight: ", prevHeight.height, "latestBlock: ", latestBlock.Height),
					}
					w.WriteHeader(http.StatusInternalServerError)
					err := json.NewEncoder(w).Encode(response)
					if err != nil {
						log.Error("Failed to write, latest height less than previous height, latest height: %d, previous height: %d", latestBlock.Height, prevHeight.height, err)
					}
					return
				}
			}
