...
func testErc721ToSubstrateRoundTrip(t *testing.T, ctx *testContext) {
	...
	ethtest.Erc721MintMany(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, tokens)
	ethtest.Erc721ApproveMany(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, ctx.ethA.BaseContracts.ERC721HandlerAddress, tokens)
	...
	for i := 1; i <= numberOfTxs; i++ {
		...
		ok := t.Run(fmt.Sprintf("Transfer %d", i), func(t *testing.T) {
			...
			ethtest.Erc721AssertOwner(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, tok.Id, ctx.ethA.Opts.From)
			ethtest.Erc721AssertMetadata(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, tok.Id, string(tok.Metadata[:]))

			// Initiate transfer
			log.Info("Submitting transaction", "number", i, "from", ctx.ethA.Opts.From, "handler", ctx.ethA.BaseContracts.ERC721HandlerAddress.String(), "tokenId", tok.Id.String())
			eth.CreateErc721Deposit(t, ctx.ethA.Client, ctx.ethA.Opts, SubChainId, subRecipient, tok.Id, ctx.ethA.BaseContracts, ctx.EthSubErc721ResourceId)
			...
			ethtest.Erc721AssertNonExistence(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, tok.Id)
		})
		if !ok {
			return
		}
	...
	for i := 1; i <= numberOfTxs; i++ {
		...
		ok := t.Run(fmt.Sprintf("Transfer %d", i), func(t *testing.T) {
			...
			// Verify ownership and intact metadata
			ethtest.Erc721AssertOwner(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, tok.Id, ethRecipient)
			ethtest.Erc721AssertMetadata(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Sub, tok.Id, string(tok.Metadata[:]))
...
func testErc721EthToEthRoundTrip(t *testing.T, ctx *testContext) {
	...
	ethtest.Erc721MintMany(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, tokens)
	ethtest.Erc721ApproveMany(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, ctx.ethA.BaseContracts.ERC721HandlerAddress, tokens)
	...
	for i := 1; i <= numberOfTxs; i++ {
		...
		ok := t.Run(fmt.Sprintf("Transfer ethA to ethB %d", i), func(t *testing.T) {
			...
			ethtest.Erc721AssertOwner(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, tok.Id, ctx.ethA.Opts.From)
			ethtest.Erc721AssertMetadata(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, tok.Id, string(tok.Metadata[:]))

			// Verify non-existence on ethB
			ethtest.Erc721AssertNonExistence(t, ctx.ethB.Client, ctx.ethB.Opts, ctx.ethB.TestContracts.Erc721Eth, tok.Id)

			// Initiate transfer
			log.Info("Submitting transaction", "number", i, "recipient", ethBRecipient, "resourcId", ctx.EthEthErc721ResourceId.Hex(), "tokenId", tok.Id.String(), "from", ctx.ethA.Opts.From, "handler", ctx.ethA.BaseContracts.ERC721HandlerAddress)
			...
			ethtest.Erc721AssertOwner(t, ctx.ethB.Client, ctx.ethB.Opts, ctx.ethB.TestContracts.Erc721Eth, tok.Id, ethBRecipient)
			ethtest.Erc721AssertMetadata(t, ctx.ethB.Client, ctx.ethB.Opts, ctx.ethB.TestContracts.Erc721Eth, tok.Id, string(tok.Metadata[:]))

			// Verify non-existence on ethA
			ethtest.Erc721AssertNonExistence(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, tok.Id)
		})
		if !ok {
			return
		}
	...
	ethtest.Erc721ApproveMany(t, ctx.ethB.Client, ctx.ethB.Opts, ctx.ethB.TestContracts.Erc721Eth, ctx.ethB.BaseContracts.ERC721HandlerAddress, tokens)
	...
	for i := 1; i <= numberOfTxs; i++ {
		...
		ok := t.Run(fmt.Sprintf("Transfer ethB to ethA %d", i), func(t *testing.T) {
			...
			// Initiate transfer
			log.Info("Submitting transaction", "number", i, "recipient", ethBRecipient, "resourceId", ctx.EthEthErc721ResourceId.Hex(), "tokenId", tok.Id.String(), "from", ctx.ethB.Opts.From, "handler", ctx.ethB.BaseContracts.ERC721HandlerAddress)
			...
			ethtest.Erc721AssertOwner(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, tok.Id, ethARecipient)
			//ethtest.Erc721AssertMetadata(t, ctx.ethA.Client, ctx.ethA.Opts, ctx.ethA.TestContracts.Erc721Eth, tok.Id, string(tok.Metadata[:]))
