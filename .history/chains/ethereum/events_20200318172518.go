...
func (l *Listener) handleDepositEvent(eventI interface{}) msg.Message {
	...
	originChainHandlerAddress := event.Topics[2].Big()
	depositID := event.Topics[3].Big() // Only item in log is indexed.
	...
	deposit, err := l.bridgeContract.BridgeCaller.GetDepositRecord(&bind.CallOpts{}, destID.Big(), depositID)
	if err != nil {
