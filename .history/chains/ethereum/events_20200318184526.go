...
const (
	...
	DepositSignature                = "Deposit(uint256,address,uint256)"
...
func (l *Listener) handleDepositEvent(eventI interface{}) msg.Message {
	log15.Debug("Handling deposit event")
	event := eventI.(ethtypes.Log)

	originChainID := event.Topics[1].Big()
	originChainHandlerAddress := event.Topics[2]
	depositID := event.Topics[3].Big()
	...
	erc20HandlerInstance, err := ERC20Handler.NewERC20Handler(cfg.contract, conn.conn)
	if err != nil {
		return nil, err
	}

	raw := &erc20HandlerInstance.ERC20HandlerRaw{
	...
	deposit, err := erc20HandlerContract.ERC20HandlerCaller .GetDepositRecord(&bind.CallOpts{}, originChainHandlerAddress, depositID)
	// deposit, err := l.bridgeContract.BridgeCaller.GetDepositRecord(&bind.CallOpts{}, originChainHandlerAddress, depositID)
	if err != nil {
	...
	return msg.Message{
		...
		Source:      l.cfg.id,
		Destination: msg.ChainId(originChainID.Uint64()),
