...
contract('Bridge - [voteDepositProposal with validatorThreshold > 1]', async (accounts) => {
    ...
    xit("depositProposal shouldn't be voted on if it has already been Finalized", async () => {
        
    });

    xit("depositProposal shouldn't be voted on if it has an Transferred status", async () => {
