...
func createWriters(t *testing.T, client *utils.Client, contracts *utils.DeployedContracts) (*writer, *writer, func(), func(), chan error, chan error) {
	...
	writerA, stopA := createTestWriter(t, createConfig(TestChainId, "bob", latestBlock, contracts), errA)
	errB := make(chan error)
	writerB, stopB := createTestWriter(t, createConfig(TestChainId, "charlie", latestBlock, contracts), errB)
	return writerA, writerB, stopA, stopB, errA, errB
}

func createTestWriter(t *testing.T, cfg *config.Config, errs chan<- error) (*writer, func()) {
	conn := newLocalConnection(t, cfg)
	stop := make(chan int)
	writer := NewWriter(cfg, conn, newTestLogger(cfg.Name), stop, errs, nil)

	bridge, err := Bridge.NewBridge(cfg.BridgeContract, conn)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWriter_start_stop(t *testing.T) {
	...
	stop := make(chan int)
	writer := NewWriter(aliceTestConfig, conn, TestLogger, stop, nil, nil)
...
// TODO: Requires PendingNonceAt which is not required for the writer itself
